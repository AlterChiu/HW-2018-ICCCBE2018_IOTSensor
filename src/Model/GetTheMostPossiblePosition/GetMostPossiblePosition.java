		...
		for (String iot[] : iotList) {
			...
			for (int eventFile = 0; eventFile < eventFileList.length; eventFile++) {
				...
				try {
					...
					int chooseRow = index / detectedGrid;
					int chooseColumn = index - chooseRow * detectedGrid;
					chooseRow = chooseRow - detectedGrid / 2;
					chooseColumn = chooseColumn - detectedGrid / 2;

					try {
						ArrayList<Integer[]> temptArray = this.temptTree.get(stationName);
						temptArray.add(new Integer[] { chooseRow, chooseColumn });
